...
OPTIONS:
 ...
 -a (--gene-list-a) FILE    Tells the script which list of genes to use in the first tier filter.
                            Optional. Looks for a file at ./gene_list_primary.tsv by default.
 -b (--gene-list-b) FILE    Tells the script which list of genes to use in the second tier filter.
...
GetOptions(
    ...
    'a|gene-list_a=s' =>\$gene_list_a,
    'b|gene-list_b=s' =>\$gene_list_b
);
...
if ($gene_list_a)
{
    open my $geneListIn, "<", "$gene_list_a" or die "Can't open $gene_list_a\n";
...
if ($gene_list_b)
{
    open my $geneListIn, "<", "$gene_list_b" or die "Can't open $gene_list_b\n";
    $second_tier_genes = gene_list->new();
    $second_tier_genes->process_gene_list($geneListIn);
    close $geneListIn;
}

else { die "Check that the file containing the gene list is valid.\n" }

$table->filter_aberrations($first_tier_genes, $second_tier_genes);